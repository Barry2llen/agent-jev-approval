...
def install_codex_hook(
    *,
    ...
    """Merge the approval Hook into a Codex hooks.json file.

    Existing Hook definitions are preserved. A matching command is updated in
    ...
    merged, changed = _merge_config(original, command=command, timeout=timeout)
...
def _merge_config(config: dict[str, Any], *, command: str, timeout: float) -> tuple[dict[str, Any], bool]:
    ...
    permission_groups = hooks.get(HOOK_EVENT_NAME)
    if permission_groups is None:
        permission_groups = []
        hooks[HOOK_EVENT_NAME] = permission_groups
    if not isinstance(permission_groups, list):
        raise CodexHookInstallError("Codex config field 'hooks.PermissionRequest' must be an array")
    ...
    for group in permission_groups:
        ...
        for index, hook in enumerate(group_hooks):
            if isinstance(hook, dict) and hook.get("type") == "command" and _is_our_command(hook.get("command"), command):
                if hook == desired_hook:
                    return merged, False
                group_hooks[index] = desired_hook
                return merged, True

    permission_groups.append({"matcher": ".*", "hooks": [desired_hook]})
    return merged, True
